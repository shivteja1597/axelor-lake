...
public class AppBaseServiceImpl extends ScriptAppServiceImpl implements AppBaseService {
  ...
  public int getProcessTimeout() {
    AppBase appBase = getAppBase();
    if (appBase == null || appBase.getProcessTimeout() < 1) {
      return 10;
    }
    return appBase.getProcessTimeout();
  }
  ...
  public String getImportErrorPath() throws IOException {
    ...
    String dir =
        String.format(
            "import_error_%s",
            getTodayDateTime().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss_SSS")));
  ...
  protected Path createTempDir(String dir) throws IOException {
    Path tmp;
    try {
      tmp = MetaFiles.getPath("tmp");
    } catch (RuntimeException e) {
      tmp = Path.of(System.getProperty("java.io.tmpdir"), "axelor");
    }
